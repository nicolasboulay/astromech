...
void trame_in_t::deserialise(const trame_binary_t & frame)
{
  ...
  i+=4;
  bar_status_init =(frame[i] >> 7) & 0xFE;
  bar_pos_courante=GET_WORD(frame[i+1],frame[i+2]);
  bar_nb_pulse_com=GET_WORD(frame[i+3],frame[i+4]);
  bar_nb_pulse_pid=frame[i+5];
  bar_error	  =frame[i+6];
  bar_sum_integ   =GET_WORD(frame[i+7],frame[i+8]);
  bar_pwm         =GET_WORD(frame[i+9],frame[i+10]);
